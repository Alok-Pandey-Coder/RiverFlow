...
const QuestionForm = ({ question }: { question?: EditableQuestion }) => {
    ...
    if (!user) {
        return (
            <div className="space-y-4 rounded-xl border border-white/20 bg-slate-950 p-6 text-center">
                <p className="text-lg font-semibold">Please log in to ask a question.</p>
                <Link
                    href="/login"
                    className="inline-flex items-center justify-center rounded-md bg-orange-500 px-4 py-2 font-semibold text-white transition hover:bg-orange-600"
                >
                    Go to Login
                </Link>
            </div>
