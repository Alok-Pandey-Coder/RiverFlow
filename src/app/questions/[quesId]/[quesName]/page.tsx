...
const Page = async ({ params }: { params: Promise<{ quesId: string; quesName: string }> }) => {
    ...
    const [questionRaw, answersRaw, upvotesRaw, downvotesRaw, commentsRaw] = await Promise.all([
    ...
    const author = await users.get<UserPrefs>(questionRaw.authorId);

    const serializeVotes = (list: typeof upvotesRaw) => ({
        total: list.total,
        documents: list.documents.map(vote => ({
            $id: vote.$id,
            type: vote.type,
            typeId: vote.typeId,
    ...
    const serializeComments = async (list: typeof commentsRaw) => {
        const documents = await Promise.all(
            list.documents.map(async comment => {
                let authorData: { $id: string; name: string; reputation: number } | null = null;

                const embedded = (comment as any).author;
                if (embedded) {
                    authorData = {
                        $id: embedded.$id,
                        name: embedded.name,
                        reputation: embedded.reputation,
                    };
                ...
                return {
                    ...
                    relativeCreatedAt: convertDateToRelativeTime(new Date(comment.$createdAt)),
                };
            })
        );

        return { total: list.total, documents };
    };

    const comments = await serializeComments(commentsRaw);
    const upvotes = serializeVotes(upvotesRaw);
    const downvotes = serializeVotes(downvotesRaw);

    const answers = {
        total: answersRaw.total,
        documents: await Promise.all(
            answersRaw.documents.map(async answer => {
                const [answerAuthor, answerCommentsRaw, answerUpvotesRaw, answerDownvotesRaw] =
                    await Promise.all([
                ...
                return {
                    ...
                    comments: await serializeComments(answerCommentsRaw),
                    upvotesDocuments: serializeVotes(answerUpvotesRaw),
                    downvotesDocuments: serializeVotes(answerDownvotesRaw),
                };
            })
        ),
    };
    ...
    const question = {
        ...
        attachmentId: questionRaw.attachmentId,
